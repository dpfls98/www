...
<?
    ...
    $sql = "select * from member where id='$userid'";//수정에 포인트/green 
    $result = mysql_query($sql, $connect);

    $row = mysql_fetch_array($result);
    //$row[id],$row{mame},...

    $hp = explode("-", $row[hp]); //explodeW짜른다?$row[hp] =010 -2222-3333
    $hp1 = $hp[0];
    $hp2 = $hp[1];
    $hp3 = $hp[2];

    $email = explode("@", $row[email]);//$row[emil]='asdf@.nate.com
    $email1 = $email[0];
    $email2 = $email[1];
    ...
    <ul class="join_notice">
     ...
     <table>
      <caption class="hidden">회원가입</caption>
     	...
     	<tr class="last">
     		<td colspan="2">
                <a href="#" class="join_end" onclick="check_input()">가입완료</a></li>
                <a href="#" class="join_c" onclick="reset_form()">다시작성</a>
            </td>
     	</tr>
     </table>
